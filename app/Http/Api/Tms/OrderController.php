...
use App\Models\Tms\TmsCommonLine;
...
//        dd($start_kilo,$end_kilo);
     ...
     * 添加常发线路
     * */
    public function addCommonLine(Request $request){
        $user_info = $request->get('user_info');//接收中间件产生的参数
        $total_user_id = $user_info->total_user_id;
        $now_time      = date('Y-m-d H:i:s',time());
        $input         = $request->all();
        ...
        $input['dispatcher']        =$dispatcher = 'oneself';
        $input['type']              =$type = 'user';
        $input['name']              =$name = 'oneself';
         **/
        $rules = [
            'dispatcher'=>'required',
        ];
        $message = [
            'dispatcher.required'=>'请填写地址',
        ];

        $validator = Validator::make($input,$rules,$message);
        if($validator->passes()) {
            $wheres['self_id'] = $self_id;
            $old_info = TmsCommonLine::where($wheres)->first();

            if($old_info){
                $data['update_time'] = $now_time;
                $id = TmsCommonLine::where($wheres)->update($data);
     ...
     * 常发线路列表
     * */
    public function commonLinePage(Request $request){
        /** 接收中间件参数**/
        $user_info     = $request->get('user_info');//接收中间件产生的参数
        $project_type       =$request->get('project_type');
     ...
     * 删除常发线路
     * */
    public function delCommonLine(Request $request,Status $status){
        $now_time     = date('Y-m-d H:i:s',time());
        $operationing =  $request->get('operationing');//接收中间件产生的参数
        $table_name   = 'tms_common_line';
        $medol_name   = 'TmsCommonLine';
        $self_id = $request->input('self_id');
        $flag    = 'delFlag';
